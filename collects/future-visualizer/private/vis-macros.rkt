  ...
  (syntax-parse stx
                [(dv name:id 
                 ...
                 (with-syntax ([(kws ...) (if (attribute param)
                                              (reverse (for/fold ([params '()]) ([p (in-list (syntax->list #'(param ...)))])
                                                (cons p 
                                                      (cons (string->keyword (symbol->string (syntax->datum p))) params))))
                                              '())])
                   (with-syntax ([fun (if (attribute param)
                     ...
                     #'(define-syntax (name sx)
                         ...
                         (syntax-parse sx
                                       [(_ kws ...)
                                        (with-syntax ([foo #'(void)] #;[(locals (... ...)) (if (attribute (kws ...))
                                                                              (for/list ([k (in-list (syntax->list #'(kw-args.kw (... ...))))]
                                                                                         [a (in-list (syntax->list #'(kw-args.v (... ...))))])
                                                                                (with-syntax ([kk (stx-keyword->id sx k)]
                                                                                              [aa a])
                                                                                  #'(define kk aa)))
                                                                              #'(void))])                            
                                          #'(λ (data)
                                              (parameterize ([current-visualization-data data])
                                                ;;define locals corresponding to kw args
                                                ;;todo
                                                ;;locals (... ...)
...
(define ex (execution (list (future 1 10.0 '(2) '() '() '()) 
                            (future 2 20.0 '() '() '() '())
                            (future 3 10.0 '(4) '() '() '())
                            (future 4 25.0 '() '() '() '()))))
;((creation-graph) ex)
